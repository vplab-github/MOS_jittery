...
function [] = fullTraj1(videoFile,Trajname,diff,numFrames,validTrajname,o)
...
%     
    ...
    for j=1:size(l,2)
        ...
        validTraj{k}(:,:,j) = traj(i:min(i+diff-1,numFrames),:);
...
% save the valid trajectory coordinates and trajectory ID in validTraj and the span of the frames  
 save(['./Data/' videoFile '/validTraj.mat'],'validTraj','trajids','excluded_ids','overlap','blockIndex');
 clear all;
end